...
digitArray :: U.UArray Int Char
digitArray = U.listArray (0, 35) digitList

signedIntegerToString :: Int -> Int -> String
signedIntegerToString base v | v < 0 = '-':(integerToString base $ -v)
                             | otherwise = integerToString base v
integerToString _ 0 = "0"
integerToString base v = reverse $ integerToString' base v
integerToString' _ 0 = ""
integerToString' base v =
  ((U.!) digitArray (v `mod` base)):(integerToString' base (v `div` base))

realToString :: Int -> Double -> String
realToString base v | v < 0     = '-':(unsignedRealToString base $ -v)
                    | otherwise = unsignedRealToString base v
unsignedRealToString :: Int -> Double -> String
unsignedRealToString base v =
  let
    f = floor v
    dp = v - fromInteger f
    dps' = realToString' base dp
    dps = if null dps' then "0" else dps'
  in
   (integerToString base (floor v)) ++ ('.':dps)

realToString' :: Int -> Double -> String
realToString' base 0 = ""
realToString' base dp = let
  dpb = dp * (fromIntegral base)
  in
   ((U.!) digitArray (floor dpb)):(realToString' base dpb)

combineSemantics :: MaybeSemantics a -> MaybeSemantics b -> MaybeSemantics b
combineSemantics (NoSemantics _) (NoSemantics a) = NoSemantics a
combineSemantics s@(Semantics {}) (NoSemantics a) = s { unSemantics = a }
combineSemantics (Semantics { semanticsAnnotationXml = l1, semanticsAnnotation = l2 })
                 s2@(Semantics {semanticsAnnotationXml = l1', semanticsAnnotation = l2'}) =
  s2 { semanticsAnnotationXml = l1 ++ l1', semanticsAnnotation = l2 ++ l2' }

transformWithSemantics :: (a -> MaybeSemantics b) -> MaybeSemantics a -> MaybeSemantics b
transformWithSemantics f x = combineSemantics x (f (unSemantics x))

transformIgnoringDefEnc :: (a -> b) -> WithNSCommon a -> WithCommon b
transformIgnoringDefEnc f (WithNSCommon c a) = WithCommon (nsCommon c) (f a)

splitAtChar cdelim l = uncurry (:) $ foldr (\c (s, l) -> if c == cdelim then ("",s:l) else (c:s, l)) ("", []) l

makeMaybeCD :: NSCommon -> Maybe String
makeMaybeCD c = do
  u <- nsCommonDefinitionURL c
  URL { url_path = p } <- importURL u
  return $ last (splitAtChar '/' p)

transformToCsymbol :: (a -> (String, String)) -> WithNSCommon a -> WithCommon AST
transformToCsymbol f (WithNSCommon c a) =
  let
    (defaultCD, symname) = f a
    cd = fromMaybe defaultCD $ makeMaybeCD c
  in
   WithCommon (nsCommon c) (Csymbol (Just cd) symname)

nameSymbol (NSCiText t) = t
nameSymbol (NSCiMGlyph xt) =
  concat . map (dropWhileEnd (`elem`" \t\r\n") . (dropWhile (`elem`" \t\r\n"))) $
    runLA (arr (const xt) //> getText) ()
nameSymbol (NSCiPresentationExpression xt) = 
  concat . map (dropWhileEnd (`elem`" \t\r\n") . (dropWhile (`elem`" \t\r\n"))) $
    runLA (arr (const xt) //> getText) ()

nameType (NSCiOther v) = v
nameType (NSStrictVariableType CiInteger) = "integer_type"
nameType (NSStrictVariableType CiReal) = "real_type"
nameType (NSStrictVariableType CiRational) = "rational_type"
nameType (NSStrictVariableType CiComplex) = "complex_polar_type"
nameType (NSStrictVariableType CiComplexPolar) = "complex_polar_type"
nameType (NSStrictVariableType CiComplexCartesian) = "complex_cartesian_type"
nameType (NSStrictVariableType CiConstant) = "constant_type"
nameType (NSStrictVariableType CiFunction) = "function_type"
nameType (NSStrictVariableType CiVector) = "vector_type"
nameType (NSStrictVariableType CiSet) = "set_type"
nameType (NSStrictVariableType CiList) = "list_type"
nameType (NSStrictVariableType CiMatrix) = "matrix_type"

symContentToPres (NSCiText _) = Nothing
symContentToPres (NSCiMGlyph el) = Just el
symContentToPres (NSCiPresentationExpression el) = Just el

transformNSCn :: Maybe Int -> NSConstantPart -> AST
transformNSCn (Just 10) cp = transformNSCn Nothing cp
transformNSCn Nothing (NSCnInteger i) = Cn (CnInteger i)
transformNSCn (Just b) (NSCnInteger i) =
  Apply (noSemCom $ Csymbol (Just "nums1") "based_integer")
        [noSemCom $ Cn $ CnInteger b,
         noSemCom $ Cs $ signedIntegerToString b i]
transformNSCn Nothing (NSCnReal r) = Cn $ CnReal r
transformNSCn (Just b) (NSCnReal r) = 
  Apply (noSemCom $ Csymbol (Just "nums1") "based_float")
        [noSemCom $ Cn $ CnInteger b, noSemCom $ Cs $ realToString b r]
transformNSCn Nothing (NSCnDouble r) = Cn $ CnDouble r
transformNSCn (Just b) (NSCnDouble r) = 
  Apply (noSemCom $ Csymbol (Just "nums1") "based_float")
        [noSemCom $ Cn $ CnInteger b, noSemCom $ Cs $ realToString b r]
transformNSCn _ (NSCnHexDouble h) = Cn $ CnHexDouble h
transformNSCn b (NSCnENotation d1 d2) =
  Apply (noSemCom $ Csymbol (Just "bigfloat1") "bigfloat")
        [noSemCom . transformNSCn b $ NSCnReal d1,
         noSemCom . transformNSCn Nothing $ NSCnInteger 10,
         noSemCom . transformNSCn b $ NSCnReal d2]
transformNSCn b (NSCnRational d1 d2) =
  Apply (noSemCom $ Csymbol (Just "nums1") "rational")
        [noSemCom . transformNSCn b $ NSCnInteger d1,
         noSemCom . transformNSCn b $ NSCnInteger d2]
transformNSCn b (NSCnComplexCartesian d1 d2) =
  Apply (noSemCom $ Csymbol (Just "complex1") "complex_cartesian")
        [noSemCom . transformNSCn b $ NSCnReal d1,
         noSemCom . transformNSCn b $ NSCnReal d2]
transformNSCn b (NSCnComplexPolar d1 d2) =
  Apply (noSemCom $ Csymbol (Just "complex1") "complex_polar")
        [noSemCom . transformNSCn b $ NSCnReal d1,
         noSemCom . transformNSCn b $ NSCnReal d2]
transformNSCn _ (NSCnConstant "π") = Csymbol (Just "nums1") "pi"
transformNSCn _ (NSCnConstant "ⅇ") = Csymbol (Just "nums1") "e"
transformNSCn _ (NSCnConstant "ⅈ") = Csymbol (Just "nums1") "i"
transformNSCn _ (NSCnConstant "γ") = Csymbol (Just "nums1") "gamma"
transformNSCn _ (NSCnConstant "∞") = Csymbol (Just "nums1") "infinity"
transformNSCn _ (NSCnConstant v) = Csymbol (Just "other") "v"
transformNSCn _ (NSCnOther v1 v2) = Csymbol (Just v1) v2

transformNSAST :: WithNSCommon NSAST -> MaybeSemantics (WithCommon AST)
transformNSAST (WithNSCommon c (NSCn cbase cdata)) =
  NoSemantics $ WithCommon (nsCommon c) (transformNSCn cbase cdata)
  
transformNSAST (WithNSCommon c (NSASTCi (NSCi mvarType symContent))) = 
  (defaultSemantics (WithCommon (nsCommon c) $
                     (ASTCi $ Ci $ nameSymbol symContent))) {
      semanticsAnnotationXml =
         (maybeToList . liftM (\vartype ->
                                head (runLA (mkqelem (mname "annotation-xml")
                                                     [sattr "encoding" "MathML-Content",
                                                      sattr "cd" "mathmltypes", sattr "name" "type"]
                                                     [mkqelem (mname "ci") [] [txt (nameType vartype)]])
                                            ())) $ mvarType) ++
         (maybeToList . liftM (\symPres ->
                                head (runLA (mkqelem (mname "annotation-xml")
                                             [sattr "encoding" "MathML-Presentation"]
                                             [arr $ const symPres]) ())) $ symContentToPres symContent)
      }

transformNSAST (WithNSCommon c (NSCsymbol mcd symtype symContent)) =
  (defaultSemantics (WithCommon (nsCommon c) $
                     (Csymbol (makeMaybeCD c) (nameSymbol symContent)))) {
      semanticsAnnotationXml =
         (maybeToList . liftM (\vartype ->
                                head (runLA (mkqelem (mname "annotation-xml") [sattr "encoding" "MathML-Content",
                                                                         sattr "cd" "mathmltypes",
                                                                         sattr "name" "type"]
                                                [mkqelem (mname "ci") [] [txt (nameType vartype)]]) ())) $ mvarType) ++
         (maybeToList . liftM (\symPres ->
                                head (runLA (mkqelem (mname "annotation-xml")
                                             [sattr "encoding" "MathML-Presentation"]
                                             [arr $ const symPres]) ())) $ symContentToPres symContent)
...
transformNSAST (WithNSCommon c (NSCs str)) = defaultSemantics (WithCommon c (Cs str))
transformNSAST (WithNSCommon c (NSApply op bvars quals operands)) =
  defaultSemantics (WithCommon c ())
-- NSApply => special
-- NSBind => special
-- NSError => direct?
-- NSCBytes => direct?
...
-- NSDeclare => special
-- NSLog => qualifier
-- NSMoment => special rule
-- NSLambda => special rule
-- NSCompose => qualifiers
-- NSMinus => special rule
-- NSRoot => quantifier
-- NSMax => quantifiers
-- NSMin => quantifiers
-- NSPlus => quantifiers
...
-- NSOr => quantifiers
-- NSXor => quantifiers
-- NSForall => quantifiers
-- NSExists => quantifiers
-- NSEq => quantifiers
-- NSGt => quantifiers
-- NTLt => quantifiers
-- NSGeq => quantifiers
-- NSLeq => quantifiers
-- NSTendsto => special args
-- NSInt => quantifiers
-- NSDiff => quantifiers
-- NSPartialDiff => quantifiers
-- NSSet => special rules
-- NSList => special rule
-- NSUnion => quantifiers
-- NSIntersect => quantifiers
-- NSCartesianProduct => quantifiers
-- NSSetDiff => special rule
-- NSCard => special rule
-- NSSum => quantifiers
-- NSProduct => quantifiers 
-- NSLimit => quantifiers
-- NSMean => quantifiers
-- NSSdev => quantifiers
-- NSVariance => quantifiers
-- NSMedian => quantifiers
-- NSMode => quantifiers
-- NSVector => special
-- NSMatrix => special
-- NSSelector => special rule
-- NSEmptySet => special rule
transformNSAST x = NoSemantics $ transformToCsymbol ((M.!) symbolMap) x
